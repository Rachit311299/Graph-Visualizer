...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function updateGraph() {
        ...
        const simulation = d3.forceSimulation(Array.from(nodes.values()))
            .force("link", d3.forceLink(links).id(d => d.id).distance(100)) // Maintain link distance
            .force("charge", d3.forceManyBody().strength(-100)) // Slightly repel nodes to prevent overlap
            .force("center", d3.forceCenter(width / 2, height / 2)) // Pull nodes towards the center
            .force("collide", d3.forceCollide().radius(50)) // Maintain some spacing between nodes
            .force("radial", d3.forceRadial(200, width / 2, height / 2).strength(0.05)) // Pull nodes back to a balanced position
            .on("tick", ticked);
